import type {
  ...
  UnifiedEndpointRequestBody,
  UnifiedEndpointResponse,
  UnifiedEndpointExample,
  UnifiedExampleHeader,
  UnifiedExamplePayload,
  UnifiedContentSchema,
  UnifiedOperationSchema,
  UnifiedMediaContent,
  UnifiedSchema,
...
function detectEnv(u: string): 'prod'|'test'|'dev'|null {
  ...
  if (/(prod|production)/.test(url)) return 'prod';
  if (/(test|sandbox|stage)/.test(url)) return 'test';
  if (/(dev|localhost)/.test(url)) return 'dev';
  return null;
}
...
function resolveRef<T = any>(spec: any, ref: string | undefined | null): T | null {
...
function pickExampleFromExamples(examples: any): unknown {
  if (!examples || typeof examples !== 'object') return undefined;
  for (const example of Object.values<any>(examples)) {
    if (example && typeof example === 'object') {
      if ('value' in example && example.value !== undefined) return example.value;
      if ('externalValue' in example && example.externalValue !== undefined) return example.externalValue;
    }
  }
  return undefined;
}

function pickSchemaExample(spec: any, schema: any): unknown {
  if (!schema) return undefined;
  const resolved = resolveSchema(spec, schema);
  if (!resolved || typeof resolved !== 'object') return undefined;
  if ('example' in resolved && resolved.example !== undefined) return resolved.example;
  if ('default' in resolved && resolved.default !== undefined) return resolved.default;
  if (Array.isArray(resolved.enum) && resolved.enum.length) return resolved.enum[0];
  return undefined;
}

function extractHeaderExample(spec: any, header: any): unknown {
  if (!header || typeof header !== 'object') return undefined;
  if ('example' in header && header.example !== undefined) return header.example;
...
function extractSchemaMeta(spec: any, schema: any, seen: Set<string> = new Set()): SchemaMeta {
  ...
  if (schema.$ref) {
    ...
    if (!meta.ref) meta.ref = short;
    if (!meta.displayType) meta.displayType = short ?? undefined;
    return meta;
  }
...
function buildParameter(spec: any, param: any): UnifiedEndpointParameter | null {
...
function dedupeParameters(params: UnifiedEndpointParameter[]): UnifiedEndpointParameter[] {
  const seen = new Set<string>();
  const result: UnifiedEndpointParameter[] = [];
...
function parameterToExampleHeader(param: UnifiedEndpointParameter): UnifiedExampleHeader {
...
function buildMediaContent(spec: any, content: any): UnifiedMediaContent[] {
  const items: UnifiedMediaContent[] = [];
...
function buildExamplePayloads(spec: any, content: any): UnifiedExamplePayload[] {
  ...
  for (const [mediaType, def] of Object.entries<any>(content ?? {})) {
    const payloadExample =
      (def && typeof def === 'object' && 'example' in def && def.example !== undefined)
        ? def.example
...
function buildResponseHeaders(spec: any, headers: Record<string, any> | undefined | null): UnifiedExampleHeader[] {
  const result: UnifiedExampleHeader[] = [];
...
function mergeExampleHeaders(...lists: Array<UnifiedExampleHeader[] | undefined>): UnifiedExampleHeader[] {
  const result: UnifiedExampleHeader[] = [];
  const seen = new Set<string>();
...
function dedupeOperationSchemas(schemas: UnifiedOperationSchema[]): UnifiedOperationSchema[] {
  const map = new Map<string, UnifiedOperationSchema>();
...
function buildRequestBody(spec: any, requestBody: any): UnifiedEndpointRequestBody | null {
  ...
  const contents = Object.entries<any>(resolved?.content ?? {}).reduce<UnifiedContentSchema[]>((acc, [mediaType, def]) => {
    if (def?.schema) {
...
function buildResponses(spec: any, responses: Record<string, any> | undefined | null): UnifiedEndpointResponse[] {
  const result: UnifiedEndpointResponse[] = [];
  for (const [status, definition] of Object.entries<any>(responses ?? {})) {
...
function mergeRequired(...lists: Array<string[] | undefined>): string[] {
...
function mergeProperties(target: Record<string, any>, source: Record<string, any> | undefined) {
  for (const [name, schema] of Object.entries<any>(source ?? {})) {
    if (!(name in target)) {
      target[name] = schema;
    }
  }
...
function resolveSchema(spec: any, schema: any, stack: Set<any> = new Set()): any {
  if (!schema || typeof schema !== 'object') return schema;
  if (stack.has(schema)) return schema;
  stack.add(schema);
  let base = schema;
  if (schema.$ref) {
  ...
  if (base.allOf) {
    ...
    const properties: Record<string, any> = { ...combined.properties };
...
function buildSchemaProperty(
  ...
  const property: UnifiedSchemaProperty = {
    name,
    type: meta.type ?? null,
  ...
  const children: UnifiedSchemaProperty[] = [];
...
function buildUnifiedSchema(spec: any, name: string, schema: any, includeNested: boolean): UnifiedSchema {
  const resolved = resolveSchema(spec, schema);
  const meta = extractSchemaMeta(spec, resolved);
...
function categorizeSchema(_name: string, schema: UnifiedSchema): 'object'|'enum' {
...
export function mapToUnified(spec: any, options: MapOptions = {}): UnifiedDoc {
  ...
  for (const [name, scheme] of Object.entries<any>(secSchemes)) {
  ...
  for (const [p, methods] of Object.entries<any>(spec?.paths ?? {})) {
    ...
    for (const [m, op] of Object.entries<any>(methods ?? {})) {
      ...
      const group = (byTag[tag] ||= { tag, summary: tagInfo?.description ?? null, endpoints: [] as any[] });
      if (!group.summary && tagInfo?.description) {
        group.summary = tagInfo.description;
      }
      const opParams = Array.isArray(op.parameters) ? op.parameters : [];
      const allParams: UnifiedEndpointParameter[] = [];
      ...
      const ep: any = {
        ...
        responses: [] as UnifiedEndpointResponse[],
        security: (op.security?.map((s: any) => Object.keys(s)?.[0])) ?? null,
        examples: null
      };
      ...
      const requestPayloads: UnifiedExamplePayload[] = ep.requestBody?.payloads ?? [];
  ...
  (unified.resources as any) = Object.values(byTag);
  ...
  const schemaGroups = {
    ...
    object: [] as UnifiedSchema[],
    enum: [] as UnifiedSchema[]
  };
  for (const [name, s] of Object.entries<any>(spec?.components?.schemas ?? {})) {
    if (Array.isArray(s.enum)) {
      const enumDescriptions = Array.isArray((s as any)['x-enum-descriptions'])
        ? ((s as any)['x-enum-descriptions'] as any[])
        : null;
      ...
      unified.components.enums.push({
        ...
        values: s.enum.map((v: any, index: number) => ({
          name: String(v),
          value: v,
